...
comparisons the paper makes (345 tests in total across all research
questions), plus a user-level clustered re-test to check that impression-level
...
def cohens_d(diff: np.ndarray) -> float:
    """Cohen's d for a paired difference: mean difference / its own standard deviation.
...
def compare(
    ...
    ``minimize=True`` flips the sign convention so that, for a "lower is
    better" metric (e.g. ERR@K, TCI@K), a positive returned ``delta``/``d``
    still means "b is better than a", matching the convention for
    ...
    return {
        ...
        "d_impression": sign * cohens_d(diff),
        "p_impression": wilcoxon_p(diff),
        "d_user": sign * cohens_d(per_user_diff),
        "p_user": wilcoxon_p(per_user_diff),
    }


EFFECT_SIZE_NEGLIGIBLE = 0.10  # |d| below this: statistically real (given enough n) but not practically meaningful
